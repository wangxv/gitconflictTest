    ...
    $.modal = function (params) {
        ...
        if (params.buttons && params.buttons.length > 0) {
            for (var i = 0; i < params.buttons.length; i++) {
                if(params.buttons[i].text==='取消'){
                    buttonsHTML += '<span class="modal-button cancel' + (params.buttons[i].bold ? ' modal-button-bold' : '') + '">' + params.buttons[i].text + '</span>';
                }else{
        ...
        var titleHTML = params.title ? '<div class="modal-title colorred">' + params.title + '</div>' : '';
...
+function ($) {
    ...
    $.openPanel = function (panel) {
        ...
        panel = panel ? $('.page.page-current').find(panel) : $('.page.page-current').find(".panel").eq(0);
    ...
    $.initSwipePanels = function () {
        ...
        function handleTouchStart(e) {
            ...
            panel = $('.page.page-current').find('.panel.panel-' + side);
            if(!panel[0]) return;
